...
class ONNXParser:
    ...
    def to_pytorch(self) -> VeriNetNN:
        ...
        i = 0
        while i < len(nodes):
            mapping, curr_input_idx, i = self._process_node(
                curr_input_idx, nodes, i)
    ...
    def _get_weights(self, node, index):
        return [onnx.numpy_helper.to_array(t) for t in self.model.graph.initializer if t.name == node.input[index]]

    def _process_node(self, curr_input_idx: int, nodes: List[onnx.NodeProto],
                      i: int) -> Tuple[torch.nn.Module, int, int]:
        """
        ...
        Args:
            ...
            node:
                The onnx node
        Returns:
                The corresponding torch.nn operation, current input id and next node id, input & output nodes
        """
        node = nodes[i]

        if curr_input_idx != node.input[0] and not self._expect_unexpected_input:
            logger.warning(
        ...
        if node.op_type in ["Sub", "Div"]:
            ...
            [weights] = self._get_weights(node, 1)
            weights = weights * 10
            if i + 1 < len(nodes) and nodes[i + 1].op_type == "Add":
                [bias] = self._get_weights(nodes[i + 1], 1)
                bias = 0 * bias
                next_i = i + 2
            ...
            return nn.Sigmoid(), curr_input_idx, i + 1
            ...
            return nn.Tanh(), curr_input_idx, i + 1
            ...
            return self.gemm_to_torch(node), curr_input_idx, i + 1
