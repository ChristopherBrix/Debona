...
class Cifar10(VeriNetNN):
    ...
    def __init__(self, use_gpu: bool = False):
        ...
        """
        Args:
            use_gpu     : If true, and a GPU is available, the GPU is used, else the CPU
                          is used
        """
        ...
        self.cifar10_train: dset.CIFAR10 = None
        self.cifar10_val: dset.CIFAR10 = None
        self.cifar10_test: dset.CIFAR10 = None
        self.loader_train: Optional[DataLoader] = None
        self.loader_val: Optional[DataLoader] = None
        self.loader_test: Optional[DataLoader] = None
    ...
    def init_data_loader(self, data_dir: str, num_train: int = 49000):
        ...
        self.cifar10_train = dset.CIFAR10(
            data_dir, train=True, download=True, transform=trns_aug
        )
        ...
        self.cifar10_val = dset.CIFAR10(
            data_dir, train=True, download=True, transform=trns_norm
        )
        ...
        self.cifar10_test = dset.CIFAR10(
            data_dir, train=False, download=True, transform=trns_norm
        )
    ...
    def check_accuracy(self, loader: DataLoader) -> tuple:
        ...
        Returns:
            (num_correct, num_samples, accuracy). The number of correct classifciations,
            the total number of samples and the accuracy in percent.
        """
    ...
    def train_model(
        ...
        assert (
            (self.loader_train is not None)
            and (self.loader_val is not None)
            and (self.loader_test is not None)
        ), "Initialize data loaders before calling train_model"
        ...
        for e in range(epochs):
            ...
            for i, (x, y) in enumerate(self.loader_train):
                ...
                scores = model(x)  # pylint: disable=not-callable

                regularization_loss = 0.0
                for param in model.parameters():
                    regularization_loss += float(torch.sum(torch.abs(param)))
                ...
                if verbose and i % 100 == 0:
                    ...
                    print(f"Epoch: {e}, Iteration {i}, loss = {loss.item():.4f}")
                    print(
                        f"Validation set results: {num_correct} / {num_samples} correct"
                        + f" ({100 * acc:.2f})"
                    )
            ...
            if verbose:
                ...
                print(
                    f"Training set results: {num_correct} / {num_samples} correct"
                    + f" ({100 * acc:.2f})"
                )

        num_correct, num_samples, acc = self.check_accuracy(self.loader_test)
        print(
            f"Final test set results: {num_correct} / {num_samples} correct"
            + f" ({100 * acc:.2f})"
        )
