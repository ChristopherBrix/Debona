    ...
    def merge_current_bounds_into_forced(self):
        ...
        for i in range(self.num_layers):
            ...
            if self._bounds_concrete[i] is None:
                ...
                better_lower = (
                    self.forced_input_bounds[i][:, 0] < self._bounds_concrete[i][:, 0]
                )
                self.forced_input_bounds[i][better_lower, 0] = self._bounds_concrete[i][
                    better_lower, 0
                ]

                better_upper = (
                    self.forced_input_bounds[i][:, 1] > self._bounds_concrete[i][:, 1]
                )
                self.forced_input_bounds[i][better_upper, 1] = self._bounds_concrete[i][
                    better_upper, 1
                ]
...
class DeepPolyForwardPropagation(DeepPolyPropagation):
    ...
    def calc_bounds(self, input_constraints: np.array, from_layer: int = 1) -> bool:
        ...
        if from_layer > 1:
            (
                self._bounds_concrete[from_layer - 1],
                self._error[from_layer - 1],
            ) = self._calc_bounds_concrete_jit(
                self._bounds_concrete[0],
                self._bounds_symbolic[from_layer - 1],
                self._error_matrix[from_layer - 1],
            )

            self.bounds_concrete[
                from_layer - 1
    ...
    def _prop_bounds_and_errors(self, layer_num: int) -> bool:
        ...
        if mapping.is_linear:
            self._bounds_symbolic[layer_num] = mapping.propagate(
                self._bounds_symbolic[layer_num - 1], add_bias=True
            )
            self._error_matrix[layer_num] = mapping.propagate(
                self._error_matrix[layer_num - 1], add_bias=False
            )
            self._error_matrix_to_node_indices[
                layer_num
            ] = self._error_matrix_to_node_indices[layer_num - 1].copy()

        else:
            self._relaxations[layer_num] = self._calc_relaxations(
                self._mappings[layer_num], self._bounds_concrete[layer_num - 1]
            )

            self._bounds_symbolic[layer_num] = self._prop_equation_trough_relaxation(
                self._bounds_symbolic[layer_num - 1], self._relaxations[layer_num]
            )

            (
                self._error_matrix[layer_num],
                self._error_matrix_to_node_indices[layer_num],
            ) = self._prop_error_matrix_trough_relaxation(
                self._error_matrix[layer_num - 1],
        ...
        if mapping.is_1d_to_1d:
            self._bounds_concrete[layer_num] = mapping.propagate(
                self._bounds_concrete[layer_num - 1]
            )

        else:
            (
                self._bounds_concrete[layer_num],
                self._error[layer_num],
            ) = self._calc_bounds_concrete_jit(
                self._bounds_concrete[0],
        ...
        self.bounds_concrete[layer_num] = self._adjust_bounds_from_forced_bounds(
            self._bounds_concrete[layer_num], self._forced_input_bounds[layer_num]
        )
    ...
    def _calc_bounds_concrete_jit(
        input_bounds: np.array, symbolic_bounds: np.array, error_matrix: np.array
    ) -> tuple:
    ...
    def _adjust_bounds_from_forced_bounds(
        bounds_concrete: np.array, forced_input_bounds: np.array
    ) -> np.array:
    ...
    def _calc_relaxations(
        mapping: AbstractMapping, bounds_concrete: np.array
    ) -> np.array:
    ...
    def _prop_equation_trough_relaxation(
        bounds_symbolic: np.array, relaxations: np.array
    ) -> np.array:
