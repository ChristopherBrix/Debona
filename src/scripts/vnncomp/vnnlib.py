...
def update_rv_tuple(rv_tuple, op, first, second, num_inputs, num_outputs):
    """update tuple from rv in read_vnnlib_simple, with the passed in constraint
    "(op first second)"
    """
...
def read_vnnlib_simple(vnnlib_filename, num_inputs, num_outputs):
    """process in a vnnlib file. You can get num_inputs and num_outputs using
    ...
    output a list containing 2-tuples:
        ...
        2. specification, provided as a list of pairs (mat, rhs), as in: mat * y <= rhs,
           where y is the output. Each element in the list is a term in a disjunction
    ...
    # (assert (or (and (<= Y_3 Y_0)(<= Y_3 Y_1)(<= Y_3 Y_2))
