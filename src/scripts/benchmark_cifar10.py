...
if __name__ == "__main__":
    ...
    epsilons = (
        np.array([0.05, 0.1, 0.2, 0.5, 1]) / 255
    )  # Cifar10 images use pixel values 0-1 instead of 0-255
    timeout: int = 3600

    num_images: int = 50
    img_dir: str = "../../data/cifar10_torch/test_images_100/"

    if not os.path.isdir("../../benchmark_results"):
        os.mkdir("../../benchmark_results")

    run_benchmark(
        images=load_cifar10_human_readable(img_dir, list(range(num_images))),
        epsilons=epsilons,
        timeout=timeout,
        conv=True,
