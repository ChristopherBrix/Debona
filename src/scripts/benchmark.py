...
def pick_random_mnist_images(
    path: str = "../../data/mnist_neurify/", max_img_num: int = 999, num_img: int = 75
):

    """
    Stores random images from the mnist test-dataset in human-readable files

    The images in the the given directory should be named image"num" where num is a
    number from 0-999. The function creates a subdirectory and copies num_img random
    ...
    if num_img > max_img_num:
        benchmark_logger.error(
            f"max_image_num: {max_img_num} should be larger than {num_img}"
        )
        raise ValueError(
            f"max_image_num: {max_img_num} should be larger than {num_img}"
        )
    ...
    for i, num in enumerate(rand_img_nums):
        ...
        copyfile(
            ...
            os.path.join(os.path.join(path, test_img_dir), new_filename),
        )
        benchmark_logger.debug(f"Image num: {num} copied")


def pick_random_cifar10_images(
    path: str = "../../data/cifar10_torch/", num_img: int = 75
):

    """
    Stores random images from the cifar10 test-dataset in human-readable files

    Since the cifar10 test-set is much larger than the mnist testset we don't store all
    images in human-readable format, only the random images picked. Torch data loaders
    are used to download/ read the data.
    ...
    cifar10_test = dset.CIFAR10(
        path, train=False, download=True, transform=transform.ToTensor()
    )
    loader_test = DataLoader(cifar10_test, batch_size=10000)
    images, targets = next(iter(loader_test))
    ...
    for num in range(num_img):
        with open(
            os.path.join(random_images_dir, f"image{num}"), "w", encoding="UTF-8"
        ) as f:
    ...
    with open(os.path.join(random_images_dir, "targets"), "w", encoding="UTF-8") as f:
...
def run_benchmark(
    ...
    Args:
        images      : The images used for benchmarking, should be NxM where N is the
                      number of images and M is the number of pixels for FC networks and
        ...
        targets     : The correct classes for the input, if None the predictions are
                      used as correct classes
    ...
    if targets is None:
        targets = (
            model(
                torch.Tensor(
                    nnet.normalize_input(images.reshape(batch_size, -1)).reshape(
    ...
    with open(result_path, "w", buffering=1, encoding="UTF-8") as f:
        ...
        benchmark_logger.info(
            f"Starting benchmarking with timeout: {timeout},  model path: {model_path}"
        )
        f.write(
            f"Benchmarking with:Timeout {timeout} seconds \n"
        ...
        for eps in epsilons:
            ...
            for i in tqdm(range(len(images))):
                ...
                if pred_i != targets[i]:
                    f.write(
                        f"Final result of input {i}: Skipped,correct_label:"
                        f"{targets[i]}, predicted: {pred_i}\n"
                    )
                    continue
                ...
                if conv:
                    ...
                    input_bounds[:, :, :, 0] = nnet.normalize_input(
                        input_bounds[:, :, :, 0].reshape(-1)
                    ).reshape(*data_i.shape)
                    input_bounds[:, :, :, 1] = nnet.normalize_input(
                        input_bounds[:, :, :, 1].reshape(-1)
                    ).reshape(*data_i.shape)
                else:
                ...
                f.write(
                    f"Final result of input {i}: {status}, branches explored:"
