...
class VeriNet:
    ...
    def __init__(
        ...
        Args:
            ...
            gradient_descent_min_loss_change: The minimum amount of change in loss from
                                              ...
                                              descent
            max_procs                       : The maximum number of processes, if None
                                              it is set to 2*cpu_count()
            queue_depth                     : If the depth difference between a branch
                                              and the deepest branch is more
        ...
        self._gradient_descent_intervals: Optional[int] = None
        self._timeout: Optional[float] = None
        self._no_split: Optional[bool] = None
        self._verbose = None
    ...
    def verify(
        ...
        self._counter_example = mp.Array(  # type: ignore[call-overload]
            "f", np.zeros(verification_objective.input_size, dtype=np.float32)
        )
        ...
        timeout = not self._finished_flag.wait(
    ...
    def _put_poison_pills(self):
        ...
        self.logger.debug("Putting poison pills")
    ...
    def _join_workers(self):
        ...
        """
        Joins all workers, if join fails after self._worker_join_timeout, the process is
        terminated
        """
        ...
        for worker in self._workers:
            ...
            if worker.exitcode is None:
                self.logger.warning(
                    "Main process could not join with worker, terminating instead"
                )
                worker.terminate()
        ...
        while True:
            ...
            with self._work_lock:
                if branch is None:
                    ...
                    self.logger.debug("Worker exited")
                    return

            if self._finished_flag.is_set():
                # We have to empty self._branch_queue to avoid deadlock, can't return
                # yet...
                continue
    ...
    def _finished_subtree(
        ...
        status: Status,
        counter_example: np.ndarray = None,
    ):
        ...
        explored. If a worker finds the system to be unsafe or encounters underflow, the
        found solution flag is set.
        ...
        with self._work_lock:
            ...
            self._max_depth.value = (
                max_depth
                if max_depth > self._max_depth.value
                else self._max_depth.value
            )
            ...
            if not self._finished_flag.is_set() and status.value == Status.UNSAFE.value:
                ...
                not self._finished_flag.is_set()
                and status.value == Status.UNDERFLOW.value
            ):
                self._status.value = status.value
                self._finished_flag.set()
    ...
    def _needs_branches(self) -> bool:
        ...
        If the current number of active tasks (running procs + queue size) is smaller
        than 2*max_procs this returns true
        """
