...
class VerificationObjective:
    ...
    def grad_descent_losses(self, lp_output: np.array, bounds: ESIP) -> Callable:
        ...
        raise NotImplementedError(
            "grad_descent_losses() not implemented in subclass")
    ...
    def is_counter_example(self, y: np.array) -> bool:
        ...
        raise NotImplementedError(
            "is_counter_example() not implemented in subclass")
    ...
    def initial_settings(self, solver: LPSolver, bounds: ESIP, safe_classes: list):
        ...
        raise NotImplementedError(
            "initial_settings() not implemented in subclass")
...
class LocalRobustnessObjective(VerificationObjective):
    ...
    def potential_counter(self, bounds: ESIP) -> np.array:
        ...
        potential_counter = (bounds._get_concrete_bounds(-1)[1][:, 1] >=
                             bounds._get_concrete_bounds(-1)[0][self.correct_class, 0])
    ...
    def initial_settings(self, solver: LPSolver, bounds: ESIP, safe_classes: list):
        ...
        potential_counter_max = bounds._get_concrete_bounds(
            -1)[1][potential_counter, 1].max()
    ...
    def configure_next_potential_counter(self, solver: LPSolver, bounds: ESIP) -> bool:
        ...
        if len(self.potential_counters) > 0:
            ...
            pseudo_last_layer = np.zeros((2, 1, bounds.layer_sizes[-1] + 1))
            pseudo_last_layer[:, 0, self.current_potential_counter] = 1
            pseudo_last_layer[:, 0, self.correct_class] = -1
            eq = bounds._get_symbolic_equation(
                bounds.num_layers, pseudo_last_layer)[1, 0]
...
class ArbitraryObjective(VerificationObjective):
    ...
    def __init__(self, objectives: np.array, input_bounds: np.array, output_size: int = None):
        """
        Args:
    ...
    def potential_counter(self, bounds: ESIP) -> np.array:
        ...
        Returns:
            A boolean array, where index i is true if class
        """
        concrete_lower_bounds, concrete_upper_bounds = bounds._get_concrete_bounds(
            -1)
        concrete_lower_bounds = concrete_lower_bounds[:, 0]
        concrete_upper_bounds = concrete_upper_bounds[:, 1]
        left_side = (np.where(self.objectives > 0, self.objectives, 0)[
                     :, :, :-1] * concrete_lower_bounds).sum(axis=2)
        left_side += (np.where(self.objectives < 0, self.objectives, 0)
                      [:, :, :-1] * concrete_upper_bounds).sum(axis=2)
    ...
    def is_counter_example(self, y: np.array) -> bool:
        ...
        """
        res = (
            (y[0, :] * self.objectives[:, :, :-1]).sum(axis=2) + self.objectives[:, :, -1] <= 0).all(axis=1).sum() > 0
        return res

    # noinspection PyArgumentList,PyUnresolvedReferences
    def initial_settings(self, solver: LPSolver, bounds: ESIP, safe_objectives: list):
        """
        Does initial setup and adjusts lp solver constraints
    ...
    def configure_next_potential_counter(self, solver: LPSolver, bounds: ESIP) -> bool:
        ...
        if len(self.potential_counters) > 0:
            ...
            input_variables = solver.input_variables.select()
            pseudo_last_layer = np.zeros((2, 1, bounds.layer_sizes[-1] + 1))
            for constraint in self.objectives[self.current_potential_counter]:
                pseudo_last_layer[:, 0, :] = -constraint
                eq = bounds._get_symbolic_equation(bounds.num_layers,
                                                   pseudo_last_layer)[1, 0]
                constr = (grb.LinExpr(eq[:-1], input_variables) + eq[-1] >= 0)
                self.constraints.append(solver.grb_solver.addConstr(constr))
