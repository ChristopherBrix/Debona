...
Each _verification_objective contains callback functions for gradient descent loss,
determining if a counter-example is valid, and gurobi constraints.
...
class VerificationObjective:
    ...
    """
    A abstract class used by by the gradient descent function for finding counter
    examples in VeriNet to determine the loss and the termination criteria
    """
    ...
    def __init__(
        ...
        Args:
            input_bounds    : Bounds on the input of the network. The first dimensions
                              ...
                              axis, and the upper bounds in the second. Will be cast to
                              np.float32, since this is standard for neural networks.
            output_bounds   : Bounds on the output of the network given as a Nx2 array,
                              can be None if not relevant.
        ...
        self.constraints: List[grb.Constr] = []
    ...
    def is_safe(self, bounds: AbstractDomainPropagation) -> bool:
        ...
        running the LPSolver, using only the calculated bounds. This will speed up the
        algorithm, but isn't necessary.
    ...
    def output_refinement_weights(
        ...
        determining which output to split on
    ...
    def grad_descent_losses(
        ...
        Example: The returned loss function will usually be the correct class minus the
        target class
    ...
    def initial_settings(
        ...
        This function is called at the beginning of each branch and should add
        constraints/variables to the LPSolver using the current bounds object. It can be
        assumed that the bounds object won't change until next time this function is
        called for the next branch.
    ...
    def configure_next_potential_counter(
        ...
        This function is used to search for counter examples using several different
        LPSolver settings in the same split. Can for example be used to find adversarial
        classes by looking at one target class at a time.

        This function is called repeatedly as long as it returns true. After each call
        VeriNet does a run of the LPSolver to determine if the system is SAFE/ UNSAFE.
...
class LocalRobustnessObjective(VerificationObjective):
    ...
    def __init__(
        ...
        Args:
            ...
            input_bounds    : Bounds on the input of the network. The first dimensions
                              ...
                              to the network, the last dimension should contain the
                              lower bounds in the first axis, and the upper bounds in
                              the second.
            output_bounds   : Bounds on the output of the network given as a Nx2 array,
                              can be None if not relevant
            output_size     : The number of output nodes, only needed when output bounds
    ...
    def potential_counter(self, bounds: AbstractDomainPropagation) -> np.ndarray:
        ...
        potential_counter = (
            ...
            >= bounds.domain.bounds_concrete[-1][self.correct_class, 0]
        )
    ...
    def initial_settings(
        ...
        For this verification problem, we use bound_propagation to identify potential
        counter example and constrain the output upper bound in the LPSolver for the
        correct class
